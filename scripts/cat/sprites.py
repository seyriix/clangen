...
class Sprites:
    ...
    def make_group(
        self, spritesheet, pos, name, sprites_x=3, sprites_y=12, no_index=False
    ...
    def load_all(self):
        ...
        if isinstance(self.size, int):
            pass
        elif width / 3 == height / 12:
            self.size = width / 3
        else:
            self.size = 50  # default, what base clangen uses
            print(f"lineart.png is not 3x12, falling back to {self.size}")
            print(
                f"if you are a modder, please update scripts/cat/sprites.py and "
                f"do a search for 'if width / 3 == height / 12:'"
            )
